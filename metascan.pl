...
#    This script was made using lots of hashes. Use at own risk.
...
my $databasedir="/path/to/metascan_databses";
...
my %tools = (
   ...
   'blastn' => {
     ...
     MINVER  => "2.1", #This is actually 2.2, but since were at 2.10 now, the comp thinks were at 2.1 again.
     NEEDED  => 1,
   },
   'blastp' => {
...
# als einde van het eiwit geen stopcodon is, dan >
   ...
   my $BLASTNCMD = "blastn -query - -db %d -evalue %e -num_threads $cpus -num_alignments 1 -num_descriptions 1 -max_hsps 1 -outfmt $blastnsettings";
...
# to iterate over multiple directories, go the root/base/home directory from where your subdirs are and run : for dir in */; do  cd $dir && /scratch/scripts_common/fastaextract_desc_id.pl *.all.faa methanol ../methanol &&  cd ..; done
